...
export function NovoProjetoModal({ open, onOpenChange, onSuccess }: NovoProjetoModalProps) {
  ...
  // Se o usuário não estiver logado, nem mostramos o dialog pra evitar bugs
  // A UI por fora quem deve impedir de abrir
  if (!usuario && open) {
    toast.error("Você precisa estar logado para publicar um projeto.");
    onOpenChange(false);
    return null;
  }
