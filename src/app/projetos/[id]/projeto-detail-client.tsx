...
type Props = {
  slug: string;
  initialData?: ProjetoWithRelations;
};

export default function ProjetoDetailClient({ slug, initialData }: Props) {
  const id = slug;
  const router = useRouter();
  const { data: raw, isLoading, error } = useProjetoBySlug(id, { initialData });
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex flex-col">
                    ...
                    <span className="text-xs text-muted-foreground">
                      Em {new Date(projeto.criadoEm).toLocaleDateString("pt-BR")}
                    </span>
                  </div>
                </div>
              );
