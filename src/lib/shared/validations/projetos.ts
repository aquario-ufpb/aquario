...
const createProjetoBaseSchema = z.object({
  ...
  // Plain markdown — render path uses react-markdown which produces React
  // nodes (not HTML strings), so embedded raw HTML inside markdown is escaped
  // as literal text. No XSS surface at the storage layer.
