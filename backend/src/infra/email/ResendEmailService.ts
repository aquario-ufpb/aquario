...
export class ResendEmailService implements IEmailService {
  ...
  async sendVerificationEmail(to: string, token: string, nome: string): Promise<void> {
    ...
    log.info('Enviando email de verificação', { to });
    ...
    try {
      ...
      log.info('Email de verificação enviado com sucesso', { to });
    } catch (error) {
      log.error('Erro ao enviar email de verificação', { to, error });
  ...
  async sendPasswordResetEmail(to: string, token: string, nome: string): Promise<void> {
    ...
    log.info('Enviando email de reset de senha', { to });
    ...
    try {
      ...
      log.info('Email de reset de senha enviado com sucesso', { to });
    } catch (error) {
      log.error('Erro ao enviar email de reset de senha', { to, error });
